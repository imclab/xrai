Shader "Custom/WiggleWaterToy"
{
      Properties
    {
		...
		_CircleCenter ("Foam Circle Center (XY in UV)", Vector) = (0.5, 0.5, 0, 0)
		_CircleRadius ("Foam Circle Radius", Float) = 0.2
    ...
    SubShader
    {
        ...
        Pass
        {
			...
			float4 _CircleCenter; // xy = center in UV space
			float _CircleRadius;
            ...
            float4 frag (v2f i) : SV_Target
            {
				...
				*/
				...
				// Calculate screen-space UV coordinate (normalized 0-1)
				float2 screenUV = i.screenPosition.xy/ i.screenPosition.w;

				// Compute distance from pixel to center of fake object circle
				float distToCenter = distance(i.noiseUV, _CircleCenter.yx + 0.25 * pow(_SurfaceNoiseScroll.yx, 1));

				// Calculate a fake "depth difference" based on distance from center and radius
				// Inside circle (dist < radius) means closer, outside means farther
				float depthDifference = saturate(( distToCenter -_CircleRadius ) / _DepthMaxDistance);

				// Convert depthDifference into a 0..1 blend factor for shallow/deep color
				...
				float2 noiseUV = float2((i.noiseUV.x + SimplexNoise(0.01*_Time.wx)) + distortSample.x, 
				(i.noiseUV.y + SimplexNoise(0.01*_Time.yz)) + distortSample.y);
