...
>
> **Note**: References to `VFXBinderManager` below are legacy. The current primary pipeline is `ARDepthSource` + `VFXARBinder`.
...
| Hologram Count | Hybrid Bridge (ARDepthSource) | Both Pipelines Active |
|----------------|------------------|----------------------|
...
**Use ARDepthSource as the single compute source**. If H3M anchor/scale features are needed, extend the Hybrid Bridge pipeline rather than running duplicate compute.
...
AR Sensors (ARKit)
       ...
       │                                                                 │
       ▼                                                                 ▼
ARDepthSource (PRIMARY)                                      HologramSource (H3M)
...
ALL VFX in Scene                                    Hologram.vfx
  ...
  ├── Audio VFX ◄── AudioBridge (legacy: EnhancedAudioProcessor)
...
| File | Change |
|------|--------|
| `Assets/Scripts/_Legacy/VFXBinderManager.cs` | Added rotation blit + RayParams flip |
| `Assets/Shaders/RotateUV90CW.shader` | New shader for UV rotation |

---

## Legacy Pipeline: VFXBinderManager (Superseded)

**File**: `Assets/Scripts/_Legacy/VFXBinderManager.cs`
...
- Delegates audio binding to `AudioBridge` (legacy: `EnhancedAudioProcessor`)
...
Lightweight audio → global shader props for VFX (primary path).

### EnhancedAudioProcessor (Legacy)
...
### VFXARDataBinder
**File**: `Assets/Scripts/_Legacy/VFXARDataBinder.cs`
**Status**: ⚠️ LEGACY - Use only if you cannot add `VFXARBinder`

Use `VFXARBinder` for runtime spawns and `ARDepthSource` for shared compute.

### ARKitMetavidoBinder
**File**: `Assets/Scripts/ARKitMetavidoBinder.cs`
**Status**: ⚠️ LEGACY - Superseded by ARDepthSource + VFXARBinder

Per-VFX binding approach. Replaced by the Hybrid Bridge pipeline.
...
Creates VFX at runtime, binds per-VFX. Anti-pattern replaced by ARDepthSource + VFXARBinder.
...
```csharp
// Scene setup (once):
// 1. Add ARDepthSource (singleton compute)
// 2. Add VFXARBinder to each VFX (or use Pipeline Master setup)
// 3. Add HandVFXController (if hand effects needed)
// 4. Add AudioBridge (if audio effects needed)
...
// VFXBinderUtility is legacy and should be avoided.
```
...
| AudioBridge (primary) / EnhancedAudioProcessor (legacy) | 1ms | FFT analysis |
...
|--------|-------------|----------------|
| **Audio** | AudioBridge (primary), EnhancedAudioProcessor (legacy) | Bass, Mid, Treble, Volume (BeatDetected legacy only) |
