...
**Note**: This spec created several legacy components later superseded by the Hybrid Bridge pipeline (`ARDepthSource` + `VFXARBinder`) and `AudioBridge`.

### Phase 1: VFX Management
- [x] Create VFXCategory.cs with binding requirements flags
- [x] Create VFXBinderManager.cs for unified data binding (legacy)
...
- [x] Create EnhancedAudioProcessor.cs with FFT frequency bands (legacy)
...
- [x] Create VFXARDataBinder.cs with full AR binding (legacy)
...
- [x] Create VFXBinderUtility.cs for preset-based setup (legacy)
