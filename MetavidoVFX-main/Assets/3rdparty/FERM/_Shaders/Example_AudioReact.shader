Shader "FERM/Example_AudioReact"
{
...
{
    for(int i = 0; i < par17; i++) {
        pos = InverseTransform(par08, par09, par10, Mirror(Mirror(Mirror(pos, par15, par16), par13, par14), par11, par12));
    }
    float toReturn = length(pos);
    for(i = 0; i < par17; i++) {
        toReturn = (toReturn * par10);
    }
    return toReturn;
}
//#
...
{
    //#function
return DeformedMix((hlp0(Revolve(InverseTransform(par05, par06, par07, RepeatXZ(pos, par04)), par18)) * par07), ((Plane(InverseTransform(par20, par21, par22, RepeatXZ(pos, par04)), par19) * par22) + Wobble(par23, par24, RepeatXZ(pos, par04))), par00, par01, par02, par03);
//#
}
