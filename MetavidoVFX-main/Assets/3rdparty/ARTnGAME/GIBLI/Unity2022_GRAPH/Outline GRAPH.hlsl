...
void Outline_float(float2 UV, float OutlineThickness, float DepthSensitivity, float NormalsSensitivity, float ColorSensitivity, float4 OutlineColor, out float4 Out, out float3 Normals, out float3 Original, out float Alpha, out float Outlines, out float SceneDepth)
{
    ...
    float halfScaleFloor = (OutlineThickness * 0.5) - OutlineThickness;
    float halfScaleCeil = (OutlineThickness * 0.5) - OutlineThickness;
	...
	float edgeColor = sqrt(dot(colorFiniteDifference0.rgb, colorFiniteDifference0.rgb) + dot(colorFiniteDifference1.rgb, colorFiniteDifference1.rgb));
    Alpha = edgeColor;
