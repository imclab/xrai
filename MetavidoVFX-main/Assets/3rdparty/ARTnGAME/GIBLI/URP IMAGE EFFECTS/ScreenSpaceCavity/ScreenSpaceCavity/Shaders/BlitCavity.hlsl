...
VaryingsB VertQuadPadding(AttributesB input)
{
    ...
    output.positionCS.xy = output.positionCS.xy * float2(2.0f, -2.0f) + float2(-1.0f, 1.0f); //convert to -1..1
    output.uv = uv;
    output.uv = (output.uv - offsetPadding) * scalePadding;
...
float4 FragBlit(VaryingsB input, SamplerState s)
{
    ...
    return SAMPLE_TEXTURE2D_ARRAY_LOD(_BlitTexture, s, input.uv.xy, _BlitTexArraySlice, _BlitMipLevel);
#endif

    UNITY_SETUP_STEREO_EYE_INDEX_POST_VERTEX(input);
    return SAMPLE_TEXTURE2D_X_LOD(_BlitTexture, s, input.uv.xy, _BlitMipLevel);
}
