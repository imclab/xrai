...
half3 ShadeGIDefaultMethod(SurfaceData surfaceData, LightingDataA lightingData)
{
...
half3 ShadeSingleLightDefaultMethod(SurfaceData surfaceData, LightingDataA lightingData, Light light)
{
...
half3 ShadeSingleLightDefaultMethodMAIN(SurfaceData surfaceData, LightingDataA lightingData, Light light)
{
    half3 N = lightingData.normalWS;
    half3 L = light.direction;
    half3 V = lightingData.viewDirectionWS;
    ...
    // light's distance & angle fade for point light & spot light (see GetAdditionalPerObjectLight() in Lighting.hlsl)
    // Lighting.hlsl -> https://github.com/Unity-Technologies/Graphics/blob/master/com.unity.render-pipelines.universal/ShaderLibrary/Lighting.hlsl
    lightAttenuation *= min(2, 1); //max intensity = 2, prevent over bright if light too close, can expose this float to editor if you wish to
...
half3 ShadeGIYourMethod(SurfaceData surfaceData, LightingDataA lightingData)
{
    return 0; //write your own equation here ! (see ShadeGIDefaultMethod(...))
}
half3 ShadeMainLightYourMethod(SurfaceData surfaceData, LightingDataA lightingData, Light light)
{
    return 0; //write your own equation here ! (see ShadeSingleLightDefaultMethod(...))
}
half3 ShadeAllAdditionalLightsYourMethod(SurfaceData surfaceData, LightingDataA lightingData, Light light)
{
    return 0; //write your own equation here ! (see ShadeSingleLightDefaultMethod(...))
}
...
half3 ShadeGI(SurfaceData surfaceData, LightingDataA lightingData)
{
    //you can switch to ShadeGIYourMethod(...) !
    return ShadeGIDefaultMethod(surfaceData, lightingData); 
}
half3 ShadeMainLight(SurfaceData surfaceData, LightingDataA lightingData, Light light)
{
    //you can switch to ShadeMainLightYourMethod(...) !
    return ShadeSingleLightDefaultMethodMAIN(surfaceData, lightingData, light); 
}
half3 ShadeAdditionalLight(SurfaceData surfaceData, LightingDataA lightingData, Light light)
{
