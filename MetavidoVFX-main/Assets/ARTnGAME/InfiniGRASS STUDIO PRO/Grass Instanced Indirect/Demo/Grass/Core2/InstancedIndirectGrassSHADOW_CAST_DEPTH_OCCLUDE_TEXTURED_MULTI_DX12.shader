Shader "MobileDrawMeshInstancedIndirect/SingleGrassSHADOW_CAST_DEPTH_OCCLUDE_TEXTURED_MULTI_DX12"
{
	Properties
	{
		...
		_GroundColor("_GroundColor", Color) = (0.5,0.5,0.5)
			_Cutoff("Alpha cutoff", Range(0,1)) = 0.5
			 ...
			 _plantTexture("_plantTexture", 2D) = "white" {}
					...
					_BurnCenter("Ocean Center", Vector) = (0, 0, 0, 0)
						...
						enableGrassShape("Enable Grass Shaping Texture", Float) = 0
							_erasedShadowFactor("Shape Erased Grass Shadow Factor", Float) = 1
							 ...
							 depthCutoffHeigth("depthCutoffHeigth", Float) = 0
	}
		...
		void Setup()
		{
			// Required for procedural instancing
		}
							struct Attributes
							{
							...
							CBUFFER_END
								...
								sampler2D _plantTexture;
								float4 _plantTexture_ST;
								...
								TEXTURE2D(_GrassBendingRT);
								SAMPLER(sampler_GrassBendingRT);
								...
								//{
								Varyings vert(Attributes IN, uint instanceID : SV_InstanceID) //v0.1
								{
									Varyings OUT;
									...
									uint realID = _VisibleInstanceOnlyTransformIDBuffer[instanceID];

									//float3 perGrassPivotPosWS = _AllInstancesTransformBuffer[realID];
									
									

									//perGrassPivotPosWS = _AllInstancesTransformBuffer[_VisibleInstanceOnlyTransformIDBuffer[instanceID]];//we pre-transform to posWS in C# now
									//uint id = _VisibleInstanceOnlyTransformIDBuffer[unity_InstanceID];
									//perGrassPivotPosWS = _AllInstancesTransformBuffer[id];
									...
									//float4 worldPos = float4(perGrassPivotPosWS, 1);

									//perGrassPivotPosWS = float3(0, 0, 0);
									...
									//v0.1
									float stepped = SAMPLE_TEXTURE2D_LOD(
										_GrassBendingRT, sampler_GrassBendingRT,
										grassBendingUV, 0
									).x;
									...
									//=========================================
									float3 cameraTransformRightWS = float3(1, 0, 0);// UNITY_MATRIX_V[0].xyz;//UNITY_MATRIX_V[0].xyz == world space camera Right unit vector
									float3 cameraTransformUpWS = float3(0, 1, 0); //UNITY_MATRIX_V[1].xyz;//UNITY_MATRIX_V[1].xyz == world space camera Up unit vector
									float3 cameraTransformForwardWS = -float3(1, 0, 1);;// -UNITY_MATRIX_V[2].xyz;//UNITY_MATRIX_V[2].xyz == -1 * world space camera Forward unit vector

									//v0.1
									cameraTransformRightWS = GetWorldSpaceViewDir(float3(1, 0, 0));
									cameraTransformUpWS = float3(0, 1, 0);
									cameraTransformForwardWS = normalize(_WorldSpaceCameraPos - perGrassPivotPosWS);
									cameraTransformForwardWS = normalize(_WorldSpaceCameraPos - perGrassPivotPosWS);
									cameraTransformRightWS = normalize(cross(float3(0, 1, 0), cameraTransformForwardWS));
									...
									//positionOS = float3(IN.positionOS.x * _GrassWidth, IN.positionOS.y / 1144, IN.positionOS.z * 5);
									//positionOS = float3(IN.positionOS.x * _GrassWidth + (sin(perGrassPivotPosWS.x * 1.2 + perGrassPivotPosWS.z * 0.2) * 1.6), 
									//	(IN.positionOS.y / 1144) + (sin(perGrassPivotPosWS.x * 1.2 + perGrassPivotPosWS.z * 0.2) * 1.6), 
									//	IN.positionOS.z * 5 + (sin(perGrassPivotPosWS.x * 53.2 + perGrassPivotPosWS.z * 12) * 11.6)) ;
									//positionOS = float3(IN.positionOS.x * _GrassWidth, IN.positionOS.y / 1144, IN.positionOS.z * 5);
									positionOS = float3(IN.positionOS.x * _GrassWidth * (sin(perGrassPivotPosWS.x * 1.2 + perGrassPivotPosWS.z * 0.2) * 1.6),
											(IN.positionOS.y / 1144) * (sin(perGrassPivotPosWS.x * 0.2 + perGrassPivotPosWS.z * 0.2) * 855+10.8), 
											IN.positionOS.z * 5 * (sin(perGrassPivotPosWS.x * 53.2 + perGrassPivotPosWS.z * 12) * 2)) ;
									...
									if (texA.r < depthCutoffHeigth || texA1.r < depthCutoffHeigth || texB.r < depthCutoffHeigth || texB1.r < depthCutoffHeigth
										|| texC.r < depthCutoffHeigth || texC1.r < depthCutoffHeigth) {
										tex.r = 1;
									}

									//v1.3
									//depthCutoffHeigth
									if (tex.r < depthCutoffHeigth) {
										tex.r = 1;
									}
									...
									//if (IN.texcoord.y > 0) {
										...
										OUT.positionWS = positionWS;// (worldPos);
										//vertex position logic done, complete posWS -> posCS
										OUT.positionCS = TransformWorldToHClip(positionWS);

										//OUT.positionCS = mul(UNITY_MATRIX_VP, worldPos); // convert to clip space
										...
										half3 baseColor = SAMPLE_TEXTURE2D_LOD(
											_BaseColorTexture, sampler_BaseColorTexture,
											float4(TRANSFORM_TEX(positionWS.xz, _BaseColorTexture), 0, 0), 0
										) * _BaseColor;
								...
								////////////////// FRAGMENT
									...
									half4 frag(Varyings IN, uint instanceID : SV_InstanceID) : SV_Target
									{
										//return float4(1,0,0,1);
									float4 baseColor = tex2D(_plantTexture, IN.uv * _plantTexture_ST.xy + _plantTexture_ST.zw) * 4;//sample mip 0 only
									...
									uint realID = _VisibleInstanceOnlyTransformIDBuffer[instanceID];
									float3 perGrassPivotPosWS = _AllInstancesTransformBuffer[_VisibleInstanceOnlyTransformIDBuffer[realID]];
									//float3 perGrassPivotPosWS = _AllInstancesTransformBuffer[_VisibleInstanceOnlyTransformIDBuffer[instanceID]];//we pre-transform to posWS in C# now
									...
									half3 N = IN.normal ;
									half3 V = IN.viewVec;// viewWS / ViewWSLength;

									//half3 baseColor = tex2Dlod(_BaseColorTexture, float4(TRANSFORM_TEX(IN.positionWS.xz, _BaseColorTexture), 0, 0)) * _BaseColor;//sample mip 0 only
									//half3 albedo = lerp(_GroundColor, baseColor, IN.positionOS.y);
									half3 albedo = baseColor;

									albedo = albedo * 1 *lerp(_GroundColor, _BaseColor, IN.positionOS.y);
									...
									// Mix the pixel color with fogColor. You can optionaly use MixFogColor to override the fogColor
									// with a custom one.
									float3 color = MixFog(lightingResult, fogFactor);




									//color.rgb = color.rgb * IN.texcoord.y;


									return half4(color, 1);
									//return half4(baseColor.rgb * color, 1);

								   // return half4(IN.color * baseColor,1);
								}
								ENDHLSL
							}
		...
		Varyings vert(Attributes IN, uint instanceID : SV_InstanceID)
		{
			...
			positionWS.y = positionWS.y*16 - 4.6;
