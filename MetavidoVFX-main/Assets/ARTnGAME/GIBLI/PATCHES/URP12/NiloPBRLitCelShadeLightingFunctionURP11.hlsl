...
half3 LightingPhysicallyBasedCelShadeMAIN(BRDFData brdfData, Light light, half3 normalWS, half3 viewDirectionWS)
{
    light.shadowAttenuation = smoothstep(0.45, 0.5, light.shadowAttenuation);//***********************cel shade add line**************************
    return LightingPhysicallyBasedCelShade(brdfData, light.color, light.direction, 1 * light.shadowAttenuation, normalWS, viewDirectionWS);
}

half4 CalculateSurfaceFinalResultColor(Varyings IN, UserSurfaceOutputData surfaceData, LightingDataA lightingData)
{
    ...
    rgb += LightingPhysicallyBasedCelShadeMAIN(brdfData, lightingData.mainDirectionalLight, lightingData.normalWS, lightingData.viewDirectionWS);
