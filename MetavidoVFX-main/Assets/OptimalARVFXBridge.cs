...
public class OptimalARVFXBridge : MonoBehaviour
{
    ...
    // Thread group sizes (queried from compute shader to avoid hardcoded values)
    ...
    private void ConfigureForIOS()
    {
        ...
        // Note: Environment depth temporal smoothing is handled automatically by ARKit on iOS
    ...
    private void InitializeComputeShader()
    {
        ...
        // Query thread group sizes from compute shader (avoids hardcoded values)
        uint z;
        depthProcessor.GetKernelThreadGroupSizes(depthToWorldKernel, out threadSizeX, out threadSizeY, out z);
    }
    ...
    private void ProcessDepthData()
    {
        ...
        // Dispatch using dynamically queried thread group sizes
        int threadGroupsX = Mathf.CeilToInt(textureResolution.x / (float)threadSizeX);
        int threadGroupsY = Mathf.CeilToInt(textureResolution.y / (float)threadSizeY);
    ...
    private void ProcessVelocityData()
    {
        ...
        {
            depthProcessor.SetVector("_LeftHandVelocity", new Vector4(handVelocities[0].x, handVelocities[0].y, handVelocities[0].z, 0));
            depthProcessor.SetVector("_RightHandVelocity", new Vector4(handVelocities[1].x, handVelocities[1].y, handVelocities[1].z, 0));
        }
    ...
    // ARKit 3D skeleton joint indices (from ARSkeletonDefinition)
    private const int LeftHandJointIndex = 63;  // Left hand wrist
    private const int RightHandJointIndex = 79; // Right hand wrist
    ...
    private void OnHumanBodiesChanged(ARHumanBodiesChangedEventArgs args)
    {
        ...
        foreach (var humanBody in args.updated)
        {
            ...
            if (joints.Length > RightHandJointIndex)
            {
                var leftHand = joints[LeftHandJointIndex];
                var rightHand = joints[RightHandJointIndex];
                
                // Calculate velocities (explicit Vector3 to float3 conversion)
                Vector3 leftPos = leftHand.anchorPose.position;
                Vector3 rightPos = rightHand.anchorPose.position;
                handVelocities[0] = ((float3)leftPos - previousHandPositions[0]) / Time.deltaTime;
                handVelocities[1] = ((float3)rightPos - previousHandPositions[1]) / Time.deltaTime;
                
                // Update previous positions
                previousHandPositions[0] = leftPos;
                previousHandPositions[1] = rightPos;
                
                // Send to VFX
                vfxGraph.SetVector3("LeftHandVelocity", (Vector3)handVelocities[0]);
                vfxGraph.SetVector3("RightHandVelocity", (Vector3)handVelocities[1]);
            }
        }
    }
    ...
    private void OnFacesChanged(ARFacesChangedEventArgs args)
    {
        ...
        {
            // Calculate face velocity (explicit conversion to avoid Vector3/float3 ambiguity)
            Vector3 currentFacePos = face.transform.position;
            Vector3 faceVelocity = (currentFacePos - (Vector3)previousFacePosition) / Time.deltaTime;
            previousFacePosition = currentFacePos;
