...
public class TestRuntimeImport : MonoBehaviour
{
    ...
    {
        IcosaApi.GetAsset(id, result =>
        {
            if (result.Ok)
            {
                IcosaApi.Import(result.Value, IcosaImportOptions.Default());
            }
        });
    }
    ...
    void Start()
    {
        ...
        {
            IcosaApi.Init(authConfig, new IcosaCacheConfig());
        }
