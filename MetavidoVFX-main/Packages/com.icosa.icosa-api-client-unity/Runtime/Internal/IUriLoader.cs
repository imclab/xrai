...
// are named after the MD5 hashes of each path.
    public class HashedPathBufferedStreamLoader : IUriLoader
    {
        ...
        public IBufferReader Load(string uri)
        {
            uri = IcosaInternalUtils.ConvertFilePathToHash(uri);
            string path = Path.Combine(uriBase, uri);
            long length = new FileInfo(path).Length;
            FileStream stream = new FileStream(path, FileMode.Open, FileAccess.Read);
            return new BufferedStreamReader(stream, bufferSize, length);
...
// Load "files" from a IcosaFormat
    public class FormatLoader : IUriLoader
    {
        ...
        public FormatLoader(IcosaFormat format)
        {
            this.format = format;
        }
        ...
        public IBufferReader Load(string uri)
        {
            ...
            {
                Debug.LogError(
                    $"No resource found in {format.resources.FirstOrDefault()?.relativePath} or {format.resources.LastOrDefault()?.relativePath} for {uri}");
            }
