...
using IcosaClientInternal.caching;
using IcosaClientInternal.model.util;

namespace IcosaClientInternal.client.model.util
{
    /// <summary>
    ...
    private const int MAX_CONCURRENT_DOWNLOADS = 4;
        ...
        public delegate void CompletionCallback(IcosaStatus status, int responseCode, byte[] responseBytes);
        ...
        public void Setup(IcosaCacheConfig config)
        {
        ...
        private IEnumerator HandleWebRequest(PendingRequest request, BufferHolder bufferHolder)
        {
            ...
            IcosaUtils.AssertTrue(webRequest.downloadHandler == handler,
...
#pragma warning restore 0162 // Don't warn about unreachable code.
            ...
            IcosaStatus status = UnityCompat.IsNetworkError(webRequest)
                ? IcosaStatus.Error(webRequest.error)
                : IcosaStatus.Success();
